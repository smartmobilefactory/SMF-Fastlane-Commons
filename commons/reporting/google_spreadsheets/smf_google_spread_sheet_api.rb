...
def _smf_meta_reporting_spreadsheet_entry_to_line(entry)
  ...
  # thus it is VERY IMPORTANT to not change the order!
  [
    entry[:date], entry[:repo], entry[:platform], entry[:branch], entry[:xcode_version], entry[:idfa],
    entry[:bitcode], entry[:swiftlint_warnings], entry[:ats], entry[:swift_version],
    entry[:iphoneos_deployment_target], entry[:macosx_deployment_target], entry[:tvos_deployment_target], entry[:watchos_deployment_target]
  ]
end
