...
security delete-generic-password -a "ed25519" -s "https://sparkle-project.org" -D "private key" "$HOME/Library/Keychains/login.keychain-db"
...
security add-generic-password    -a "ed25519" -s "https://sparkle-project.org" -D "private key" -T ./generate_appcast -w "$APPCAST_PASSWORD" "$HOME/Library/Keychains/login.keychain-db"
# Using the team id, authorise the identity codesigning the generate_appcast to access the private key.
security set-generic-password-partition-list -a "ed25519" -s "https://sparkle-project.org" -k "$KEYCHAIN_PASSWORD" -S "teamid:$TEAM_ID" "$HOME/Library/Keychains/login.keychain-db"
