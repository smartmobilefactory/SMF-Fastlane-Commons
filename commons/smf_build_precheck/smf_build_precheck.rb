...
def _check_common_project_setup_files
  ...
  current_submodule_branch = `cd #{submodule_directory}; git rev-parse --abbrev-ref HEAD`.gsub("\n", '')

  UI.message("Current CommonProjectSetupFiles branch: #{current_submodule_branch}")
  if current_submodule_branch != 'master' #&& current_submodule_branch != 'HEAD'
    ENV['DANGER_COMMON_PROJECT_SETUP_FILES_WRONG_BRANCH'] = current_submodule_branch
  end
  ...
  UI.message("CommonProjectSetupFiles remote commit: #{remote_head_commit}")
