...
private_lane :smf_super_shared_setup_dependencies do |options|
  ...
  sh("#{get_flutter_binary_path} doctor")
  sh("#{get_flutter_binary_path} packages get")
...
private_lane :smf_super_ios_build do |options|
  ...
  sh("#{get_flutter_binary_path} build ios --release --no-codesign --flavor #{build_variant}")
...
private_lane :smf_super_android_build do |options|
  build_variant = !options[:build_variant].nil? ? options[:build_variant] : smf_get_first_variant_from_config
  sh("#{get_flutter_binary_path} build apk --release --flavor #{build_variant}")
  if !options[:build_variant].nil?
    sh("#{get_flutter_binary_path} build appbundle --release --flavor #{build_variant}")
  end
...
private_lane :smf_super_run_unit_tests do |options|
  sh("#{get_flutter_binary_path} test")
end
...
private_lane :smf_super_linter do |options|
  sh("#{get_flutter_binary_path} analyze || true")
end
